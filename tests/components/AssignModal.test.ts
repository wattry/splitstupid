...
function mount(desc = 'Beer', assigned: string[] = ['sam']) {
  const onToggle = vi.fn(); const onManage = vi.fn(); const onClose = vi.fn();
  const host = document.createElement('div'); document.body.appendChild(host);
  act(() => { createRoot(host).render(React.createElement(AssignModal, { desc, participants: people, assigned, onToggle, onManage, onClose })); });
